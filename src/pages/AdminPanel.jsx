...
function AdminPanel() {
    ...
    const sendConfirmationEmail = async (name, email, paperTitle, currentData) => {
        try {
            await emailjs.send(
                'service_tl2k8ng',
                'template_s4m95cn',
    ...
    const sendRejectionEmail = async (paper, rejectReason) => {
        try {
            await emailjs.send(
                'service_tl2k8ng',
                'template_s4m95cn',
                    ...
                    {papers.map((paper) => (
                        <div key={paper.id} className="paper-card">
                            ...
                            <div className="container">
                                ...
                                <div className="assign-editor-container">
    <label htmlFor="editorDropdown">Assign Editor:</label>
    <select
        id="editorDropdown"
        onChange={handleEditorChange}
        value={selectedEditor || ""} // Controlled component to manage selection
    >
        <option value="">Select an editor</option>
        {editorialTeam.map((editor) => (
            <option id={editor.id} key={editor.id} value={editor.id}>
    ...
    <button 
        ...
        disabled={!selectedEditor} // Disable button if no editor is selected
    >
        Assign
    </button>
</div>
